...
// [1,2,3,4].symmetric_difference([2,3]) ---> [ 1, 4 ]
...
// Array#difference This way, you will get an array containing all the elements of arr1 that are not in arr2 and vice-versa
// [1,2,3,4].difference([3]) ----> [1,2,4]
indexes.symmetricDifference = Array.prototype.symmetric_difference = function (array2){
    return this.filter(x => !array2.includes(x)).concat(array2.filter(x => !this.includes(x)));
}

// Array#isEqual --> compare if array equal other array or not
// [1,2].isEqual([1,2]) ---> true
// [1,2].isEqual([1,2,3]) ---> false
// [1,2,3].isEqual([1,2,4]) ---> false
indexes.isEqual = Array.prototype.isEqual = function (array2){
    return this.length === array2.length && this.every(function(value, index) { return value === array2[index]})
}
